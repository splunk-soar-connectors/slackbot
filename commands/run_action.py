...
class RunActionCommand(Command):
    ...
    def configure_parser(self, parser) -> None:
        ...
        asset_group.add_argument('-n', '--asset-name',
                                 help=f'Name of the asset to run the action on. {no_asset_message}')
        asset_group.add_argument('-i', '--asset-id', type=int,
                                 help=f'ID of the asset to run the action on. {no_asset_message}')

        parser.add_argument('--name', default='Slack generated action', help='Name for the action run')
        parser.add_argument('--type', dest='action_type', default='soarbot',
                            help='Type of action run')
    ...
    def _process_args(self, parsed_args) -> Result:
        ...
        if asset:
            ...
            create_target_result = self._create_target(action=action_info,
                                                       app_id=app_id,
                                                       asset_ids=[asset['id']],
        ...
        # If no asset argument was passed, we need to find all actions that have the given name
            ...
            for action in action_list:
                ...
                create_target_result = self._create_target(action=action,
                                                           app_id=app_id,
                                                           asset_ids=asset_ids,
                                                           parameters=parameters)
                if not create_target_result.success:
