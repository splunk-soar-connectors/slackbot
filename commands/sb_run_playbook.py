...
class RunPlaybookCommand(Command):
    ...
    def configure_parser(self, parser) -> None:
        ...
        playbook_group.add_argument("-i", "--id", dest="playbook_id", type=int, help="ID of the playbook to run")
        parser.add_argument("-r", "--repo", help="Name of the repo the playbook is in (required if playbook argument is a name, and not an ID)")
    ...
    def execute(self, parsed_args) -> str:
        ...
        try:
            playbook_run_request = self.slack_bot._soar_post(SoarRestEndpoint.PLAYBOOK_RUN, body=request_body)
        ...
        return "\n".join(
            [f"Playbook: {playbook_id}", f"Playbook run ID: {run_id}", "Playbook queueing result: Playbook run successfully queued"]
        )
