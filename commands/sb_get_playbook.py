...
class GetPlaybookCommand(Command):
    ...
    def configure_parser(self, parser) -> None:
        ...
        parser.add_argument("--status", choices=["active", "inactive", "draft"], type=str.lower, help="The playbook status to filter on")
        parser.add_argument("--type", dest="playbook_type", help="The playbook type to filter on. Case insensitive")
        parser.add_argument("--repo", help="The repo name to filter on")
        parser.add_argument("--sort-by", default="name", choices=["name", "type"], type=str.lower, help="The sort key to use")
        parser.add_argument("--sort-order", default="asc", choices=["asc", "desc"], type=str.lower, help="The sort order to use")
    ...
    def _query_playbooks(self, parsed_args) -> Result:
        ...
        query_parameters = {
            ...
            "_filter_playbook_type__icontains": self.slack_bot._create_query_string(getattr(parsed_args, "playbook_type", None)),
        ...
        try:
            get_playbook_request = self.slack_bot._soar_get(SoarRestEndpoint.PLAYBOOK, query_parameters=query_parameters)
        ...
        for playbook in playbook_info["data"]:
            ...
            try:
                ...
                if parsed_args.short:
                    message_list.append(f" Type: {playbook_type}".ljust(20) + f" Name: {playbook_name}")
                else:
