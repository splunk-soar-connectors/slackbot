...
class SlackBot:
    ...
    def _soar_get(self, endpoint: SoarRestEndpoint, query_parameters: Optional[dict] = None, path_postfix: str = ""):
        ...
        return requests.get(url, headers=self.headers, auth=self.auth, verify=self.verify, timeout=SLACK_BOT_DEFAULT_TIMEOUT)

    def _soar_post(self, endpoint: SoarRestEndpoint, body: Optional[dict] = None):
        """Make a SOAR POST request."""
        url = endpoint.url(self.base_url)

        logging.debug("Sending POST request to SOAR URL: %s", url)
        return requests.post(url, json=body, headers=self.headers, auth=self.auth, verify=self.verify, timeout=SLACK_BOT_DEFAULT_TIMEOUT)
    ...
    def _get_apps_by_app_ids(self, app_ids) -> Result:
        ...
        try:
            get_apps_request = self._soar_get(SoarRestEndpoint.APP, query_parameters=query_parameters)
    ...
    def _get_assets_by_app_ids(self, app_ids) -> Result:
        ...
        try:
            get_assets_request = self._soar_get(SoarRestEndpoint.ASSET, query_parameters=query_parameters)
    ...
    def _post_message(self, message, channel, code_block=True):
        ...
        if message:
            if len(message) <= SLACK_BOT_JSON_MESSAGE_LIMIT:
                body["text"] = f"```{message}```" if code_block else message
    ...
    def _create_parser(self, channel):
        parser = argparse.ArgumentParser(exit_on_error=False, prog="@<bot_username>", formatter_class=argparse.ArgumentDefaultsHelpFormatter)

        subparsers = parser.add_subparsers(title="commands")
        for slack_bot_command in AVAILABLE_COMMANDS:
            subparser = subparsers.add_parser(
                slack_bot_command.COMMAND_NAME,
...
def main():
    ...
    try:
        ...
        if not isinstance(sat, str):
            logging.error("The SOCKET_TOKEN entry in the bot_config file appears to not be a string")
            fail = True
    except Exception:
